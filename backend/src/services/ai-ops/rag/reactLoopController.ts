 ...
 */
const REACT_LOOP_PROMPT = `你是一个 AIOps 智能运维助手，使用 ReAct 方法解决问题。
...
Thought: 用户想查看所有规则
Action: device_query
Action Input: {"command": "<某个可能返回大量数据的路径>"}
（后果：可能返回数百条记录，导致数据截断）

**正确示范 ✅：**
Thought: 用户想查看规则列表。这可能包含大量数据，我需要分批处理。先获取前20条。
Action: device_query
Action Input: {"command": "<路径>", "proplist": "<关键字段>", "limit": 20}

（获取下一批时）
Thought: 已获取前20条，继续获取下一批。
Action: device_query
Action Input: {"command": "<路径>", "proplist": "<关键字段>", "limit": 20, "offset": 20}
 ...
 */
const KNOWLEDGE_FIRST_REACT_PROMPT = `你是一个 AIOps 智能运维助手，使用 ReAct 方法解决问题。
...
7. device_query 和 execute_command 的 command 参数是设备 API 路径格式
8. device_query 支持 proplist、limit、offset 参数控制返回数据量
9. 需要执行清理、删除、添加、修改等写操作时，必须使用 execute_command，不要使用 device_query
10. 思考内容必须具体，说明你要查询什么、为什么
11. 回答时使用中文，并基于知识库经验和设备的实际情况给出建议
...
## 分批处理示例
Thought: 用户想查看规则列表。规则可能很多，我需要分批获取。先获取前20条。
Action: device_query
Action Input: {"command": "<路径>", "proplist": "<关键字段>", "limit": 20}

## 遵循知识库步骤示例
假设知识库返回了操作步骤：
"步骤1: 使用 <某路径> 查询数据，proplist=<字段列表>"

正确做法：
Thought: 知识库 [KB-xxx] 提供了操作步骤，步骤1要求查询指定数据并使用 proplist 参数。我将严格按照步骤执行。
Action: device_query
Action Input: {"command": "<知识库指定的路径>", "proplist": "<知识库指定的字段>"}

错误做法（不要这样做）：
Thought: 我自己决定查询什么数据。
Action: device_query
Action Input: {"command": "<自己猜测的路径>"}  // 错误：没有按照知识库步骤执行`;
 ...
 */
const PARALLEL_REACT_PROMPT = `你是一个 AIOps 智能运维助手，使用 ReAct 方法解决问题。
...
\`\`\`
Thought: 我需要同时获取多项设备状态信息，这些查询相互独立，可以并行执行。

Action 1: device_query
Action Input 1: {"command": "<路径A>"}

Action 2: device_query
Action Input 2: {"command": "<路径B>"}
\`\`\`
 ...
 * @param tickDeviceId 可选的 tick 上下文推断的目标设备 ID
 */
      ...
      while (iterations < effectiveConfig.maxIterations) {
        ...
        if (isDuplicate) {
          ...
          if (isCapabilityEnabled('reflection')) {
            ...
            // 反思能力未启用，直接执行
            observation = await this.executeAction(action.toolName, action.toolInput, effectiveInterceptors, executionContext?.tickDeviceId);
          }
        }
        ...
        if (pendingVerificationDirective && stepsWithoutVerification >= VERIFICATION_AUTO_TRIGGER_STEPS) {
          ...
          try {
            ...
            const autoVerifyParams = { ...vd.verify_params, deviceId: executionContext?.tickDeviceId } as IntentParams;
...
使用最严格且快速的规则匹配进行响应，尽快给出 Final Answer 或最关键的单一 Action，避免使用耗时的工具。`;
  ...
  private async buildReActPromptAsync(message: string, steps: ReActStep[], ragContext?: RAGContext, conversationContext?: Record<string, unknown>): Promise<string> {
    ...
    if (this.config.knowledgeEnhancedMode) {
      ...
      const prompt = template
        .replace('{{device_info}}', this.getDefaultDeviceInfo())
        .replace('{{api_format_guide}}', this.getDefaultApiFormatGuide())
        .replace('{{api_reference}}', this.getDefaultApiReference())
        .replace('{{message}}', message)
  ...
  /**
   * 获取默认设备信息（当无法从 DeviceManager 获取时的回退值）
   */
  private getDefaultDeviceInfo(): string {
    return `- 设备类型: 通过 device_query/execute_command 工具管理的网络设备
- 系统版本: 可通过 monitor_metrics 获取
- API 协议: 设备 API（路径格式）`;
  }
   ...
   * 获取默认 API 格式指南
   */
  private getDefaultApiFormatGuide(): string {
    return `设备 API 使用路径格式，不是 CLI 命令格式：
- 正确格式: 使用纯路径如 /interface, /ip/address 等
- 错误格式: show ip route, /interface print（不要加 print，API 会自动添加）
- 必须使用具名参数: /ping address=8.8.8.8 count=4（不支持位置参数）
- 布尔值必须写全: disabled=yes（不能省略值）

具体可用的 API 路径请通过 knowledge_search 工具查询，或参考设备能力清单。`;
  }

  /**
   * 获取默认 API 路径参考（知识优先模式使用）
   */
  private getDefaultApiReference(): string {
    return `请使用 knowledge_search 工具查询可用的 API 路径参考。
对于可能返回大量数据的路径，必须使用 limit 和 proplist 参数进行分批处理。`;
  }
...
请使用 limit 和 proplist 参数重新查询以获取完整数据！`;
  ...
  private async summarizeWithLLM(
    ...
    // 构建摘要提示词
    const prompt = `你是一个数据分析助手。用户正在查询设备信息，工具返回了大量数据。
请提炼出与用户问题最相关的关键信息。
...
请直接输出摘要内容，不要添加额外的解释。`;
  ...
  private buildFinalAnswerPrompt(message: string, steps: ReActStep[]): string {
    ...
    return `你是一个专业的 AIOps 智能运维助手。请基于收集到的**实际数据**生成回答。
...
请直接输出回答内容，不要包含"Final Answer:"前缀。`;
  ...
  private async generateForcedFinalAnswer(
    ...
    if (adapter) {
      try {
        const prompt = `你是一个专业的 AIOps 智能运维助手。用户提出了一个请求，你已经执行了多个工具调用来收集数据。
...
请直接输出回答内容，不要包含"Final Answer:"前缀。`;
    ...
    switch (failureType) {
      case 'parameter_error':
        ...
        if (toolName === 'device_query') {
          causes.push('设备 API 路径格式错误');
          causes.push('查询命令语法不正确');
        }
        if (toolName === 'execute_command') {
          causes.push('设备 API 路径格式错误或命令不存在');
  ...
  private async generateSuggestions(
    ...
    switch (failureType) {
      case 'parameter_error':
        ...
        if (toolName === 'device_query') {
          suggestions.push('使用正确的设备 API 路径格式（可通过知识库或设备能力清单查询）');
        ...
        if (toolName === 'execute_command') {
          suggestions.push('使用正确的设备 API 路径格式（可通过知识库或设备能力清单查询）');
          suggestions.push('将路径和参数合并为 CLI 格式命令（如 <路径> <参数名>=<值>）');
          if (params.command && typeof params.command === 'string') {
            const cmd = params.command as string;
            if (cmd.includes('print')) {
    ...
    if (typeof modifiedParams.command === 'string') {
      ...
      if (originalCommand.includes('print')) {
        ...
        modifications.push({
          ...
          newValue: newCommand,
          reason: '移除 print 关键字，设备 API 会自动处理',
        });
      }
  ...
  /**
   * 获取默认的 proplist（回退默认值）
   * 当设备 CapabilityManifest 不可用时，使用此硬编码映射作为回退。
   * 这些是常见设备 API 路径的推荐查询字段，用于超时修正时减少数据量。
   * TODO: 优先从 CapabilityManifest 动态获取 proplist 建议
   */
  ...
  /**
   * 应用资源错误修正（回退默认值）
   * 常见的路径修正映射，用于自动纠正 API 路径格式错误。
   * TODO: 优先从 CapabilityManifest 动态获取路径修正规则
   */
  ...
  async executeWithReflection(
    ...
    if (!isCapabilityEnabled('reflection')) {
      // 反思能力未启用，直接执行
      const result = await this.executeAction(toolName, params, effectiveInterceptors, tickDeviceId);
    ...
    while (retryCount <= effectiveMaxRetries) {
      // 执行工具
      const result = await this.executeAction(toolName, currentParams, effectiveInterceptors, tickDeviceId);
      lastResult = result;
